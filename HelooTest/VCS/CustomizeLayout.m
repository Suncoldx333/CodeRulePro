...
@interface CustomizeLayout()<UIGestureRecognizerDelegate>
...
@implementation CustomizeLayout
    ...
    for (UICollectionViewCell *cell in self.collectionView.visibleCells) {
        ...
        if (CGRectContainsPoint(cellInCanvasFrame, pointPressedInCanvas)) {
            if ([cell isKindOfClass:[CustomizeCell class]]) {
                [(CustomizeCell *)cell setIsDragging:YES];
            }
...
- (void)endDraggingAction:(BLRearangeableBundle *)bundle {
    ...
    if ([bundle.sourceCell isKindOfClass:[CustomizeCell class]]) {
        [(CustomizeCell *)bundle.sourceCell setIsDragging:NO];
    }
...
- (void)shakeAllCell{
    ...
    for (UICollectionViewCell *cell in cells) {
        ...
        [(CustomizeCell *)cell setIsEditing:YES];
        /**如果加了shake动画就不用再加了*/
...
- (void)stopShakeAllCell{
    ...
    for (UICollectionViewCell *cell in cells) {
        ...
        if ([cell isKindOfClass:[CustomizeCell class]]) {
            [(CustomizeCell *)cell setIsEditing:NO];
        }
    }
